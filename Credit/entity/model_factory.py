...
MetricInfoArtifact = namedtuple("MetricInfoArtifact",
                                ["model_name", "model_object", "auc_score", "F_score", "Precision",
                                 "Recall", "model_accuracy", "index_number"])



def evaluate_classification_model(model_list: list, X_train:np.ndarray, y_train:np.ndarray, X_test:np.ndarray, y_test:np.ndarray, base_accuracy:float=0.7)->MetricInfoArtifact:
    pass


def evaluate_regression_model(model_list: list, X_train:np.ndarray, y_train:np.ndarray, X_test:np.ndarray, y_test:np.ndarray, base_accuracy:float=0.7) -> MetricInfoArtifact:
    """
    Description:
    ...
    try:
        ...
        for model in model_list:
            ...
            #Getting prediction for testing dataset
            ...
            #Calculating the auc and f-score
            auc_score = roc_auc_score(y_test, y_pred)
            F_score = fbeta_score(y_test, y_pred, beta=2, average='weighted', zero_division=1)
            ...
            logging.info(f"Precision\t\t Recall\t\t Model Score")
            logging.info(f"{Precision}\t\t {Recall}\t\t{model_accuracy}")
            logging.info(f"AUC Score value: [{auc_score}].")
            logging.info(f"F-Score value: [{F_score}].")





            #if model accuracy is greater than base accuracy and Recall score is within certain thershold
            #we will accept that model as accepted model
            if model_accuracy >= base_accuracy and Recall > 0.7:
